...
# Se cuenta con dos datasets diferentes, los cuales se pueden vincular mediante la columna `id_usuario`. Se procede con la union de ambos.
...
# Previo a responder los interrogantes mencionados, se realiza un análisis general para poder dar un enfoque sobre las variables que se relacionarán mas adelante.
...
# #### ¿Como se distribuye el género entre los encuestados?
...
# En esta ocasión se observa una cantidad mayoritaria de hombres encuestados. Esta diferencia fue destacada como motivo de análisis y se desarrollará en profundidad.

# #### ¿Que sede y tipo de sala predomina entre los encuestados?
...
# Dado que la columna `fila` presenta una gran cantidad de missing values (porcentaje), se optó por eliminarla del dataframe.
...
# TODO: ~~como se distribuyen los encuestados segun tipo de sala~~ Se desarrolla mas arriba
...
# #### ¿Sucede un comportamiento similar teniendo en cuenta la sede elegida?
...
# Podemos ver que no hay un comportamiento inesperado para ninguna sede en particular, la diferencia entre respuestas positivas y negativas de la sala 4D se acentúa en la sede Palermo, pero esto se debe a que la mayoria de encuestados acudió a dicha sede.
...
# #### ¿Existe cierto rango etario que quiera volver a ver Frozen 4?
...
# #### ¿Existe un rango etario para algún género que permita categorizarlo como que volvería a ver Frozen 4?

# ## TODO redactar mejor eso
...
# Sin embargo, dada la infica cantidad de encuestados hombres que fue al cine con 3 o más acompañantes o mujeres que fueron con 4 o más, se descarta el uso de estas relaciones para el baseline ya que las mismas podrían causar overfitting. 
#
# *La cantidad de encuestados según numero de acompañantes puede verse en [el análisis de los acompañantes](#Columnas-parientes-y-amigos).*

# TODO: ~~Aclarar que no es significativamente importante en los hombres~~ Creo que ya está, podrian agregarse porcentajes para que quede mas clara la idea
...
# Como se ve, la sala 4D es la mas elegida entre los encuestados, esto corresponde a lo visto en el análisis general y no difiere su comportamiento al separalos por género.
...
# #### ¿Existe, en el género femenino, algun comportamiento distintivo para algun tipo de sala o sede?
...
# TODO: ~~Poner las conclusiones junto con el grfico~~ Creo que es una buena conclusión.
...
# En busca de ampliar la relación entre las respuestas de los encuestados y el tipo de sala, se buscó analizar si el precio influía al elegir algun tipo en particular:
...
# No puede extraerse una conclusión del gráfico resultante. El comportamiento es similar para ambos géneros y no se refleja en él algún rango etario especifico para los tipos de sala o alguna característica que influencie en la decisión de volver o no a ver Frozen 4.
...
#
# Aún corriendo riesgo de sobreajuste al dataset debido a la poca cantidad de encuestados en esta categoria, se decidio incluir en el baseline este comportamiento con el objetivo de incluir una condición externa al géreno.
...
# TODO: ~~Cambiar moneda cargada porque no queremos un random dentro del codigo. Tener algo random dentro del baseline es un mal caso. Queremos que sea deterministico.~~ Ver si agregar esto como comentario anecdotico.
...
# TODO: ~~Se debe agarrar cada una de las afirmaciones y pasarlas a un gráfico.~~ Creo que ya está hecho
#
# TODO GENERAL: ~~Pasar al esquema "pregunta, analisis, respuesta"~~ IDEM
