...
# Existen solamente 3 columnas que presentan *missing values* y estas son: `fila`, `edad` y `nombre_sede`. Sin embargo esta última solo presenta 2 entradas con valores nulos.
...
# Como se observa, la mayoria de los encuestados concurrieron sin ningun pariente o amigo. Más adelante se desarrollará en profundidad esta categoría.
...
# Al combinar la cantidad de amigos y parientes en una sola columna, no se observa un comportamiento diferente. Se puede destacar que para los encuestados que fueron con entre uno y tres acompañantes, el porcentaje de aquellos que deciden volver es mayor. Sin embargo, estos representan sólo el 30% de la cantidad de encuestados en total, por lo que no parece ser estadísticamente representativo.
#
...
# Como se puede observar, la cantidad de mujeres que pagaron precio 1 es baja y a su vez la proporción entre las que volverían y no es similar. En cambio, en el caso de los hombres la amplia mayoría decide no volver. Como inicialmete se decidió categorizar a los hombres como que deciden no volver y a las mujeres como que si lo hacen, este análisis no nos permite mejorar dicha calificación.
